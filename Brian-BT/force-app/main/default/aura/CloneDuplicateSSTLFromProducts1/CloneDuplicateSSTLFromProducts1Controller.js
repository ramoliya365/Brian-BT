    ...
    doInit: function (component, event, helper) {
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS") {
                ...
                var strTest = result;
               var resultArray = strTest.toString().split('-');
     ...
     /*   var productsList = component.get("v.productsList");
        ...
        if(checkStatus){
            if(rfqRecordList.length){
                for(var k=0;k<rfqRecordList.length;k++){
                    if(rfqRecordList[k]['isSelected']){
                        for (var i = 0; i < rfqRecordList[k]['groupedRecords'].length; i++) {
                            //   component.find("checkRFQ")[i].set("v.checked", true);
                            var Id = rfqRecordList[k]['groupedRecords'][i].Id;
                            rfqRecordList[k]['groupedRecords'][i]['isSelected'] = true;
                            rfqRecordList[k]['isSelected'] = true;
                            
                            if(recordIds.indexOf(Id) == -1){
                                recordIds.push(Id)
                            }
                            rfqRecordList[k]['groupedRecordsTmp'][i]['isSelected'] = true
                ...
                for(var k=0;k<rfqRecordList.length;k++){
                    if(!rfqRecordList[k]['isSelected']){
                        for (var i = 0; i < rfqRecordList[k]['groupedRecords'].length; i++) {
                            //   component.find("checkRFQ")[i].set("v.checked", true);
                            var Id = rfqRecordList[k]['groupedRecords'][i].Id;
                            rfqRecordList[k]['groupedRecords'][i]['isSelected'] = false;
                            rfqRecordList[k]['isSelected'] = false;
                            if(recordIds.indexOf(Id) > -1){
                                recordIds.splice(recordIds.indexOf(Id),1)
                            }
                            /*var singleTempRec =  rfqRecordList[k]['groupedRecordsTmp'][i];
                            for(var j =0 ;j<singleTempRec.length;j++){
                                 rfqRecordList[k]['groupedRecordsTmp'][i][j]['isSelected'] = false
                            }*/
                        }
                    }
                }
